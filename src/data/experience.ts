    ...
    details: [
      {
        ...
        implementation: [
          '초기 라우팅은 로그인 여부나 권한 상태에 따라 `GoRouter`에서 공통으로 분기했습니다.',
          '화면 간 이동은 `route name` 기준으로 호출하고, 필요한 값은 `path parameter`와 `extra`로 전달했습니다.',
          '네트워크 요청은 `Dio` 기반 API 클래스로 분리하고, 화면에서 `provider`를 통해 호출하도록 했습니다.',
        ],
        result: [
          '공통 진입 규칙은 라우터에서, 데이터 요청 구조는 API 영역에서 처리하도록 했습니다.',
          '화면에서는 이동과 요청 호출만 담당하도록 했습니다.',
        ],
      },
      {
        ...
        implementation: [
          ...
          '응답 모델은 역직렬화 중심으로, 요청 모델은 직렬화 중심으로 구성했습니다.',
          '상태를 표현하거나 불변 객체로 다루는 값에는 `freezed`를 사용하고, 일반적인 API 모델은 `json_serializable`로 관리했습니다.',
          '중첩 응답 구조는 하위 모델로 나눠 필요한 형태로 분리했습니다.',
        ],
        result: [
          '화면이나 API 호출부에서 JSON 값을 직접 다루지 않고, 모델 단위로 변환해 사용하도록 했습니다.',
        ],
      },
      {
        ...
        implementation: [
          ...
          '현재 검색 조건과 페이지 번호를 함께 전달해 조건에 맞는 목록을 조회했습니다.',
          '첫 페이지에서는 전체 건수를 함께 갱신하고, 이후 페이지는 목록만 추가하도록 했습니다.',
          '페이지 정보와 목록 데이터를 함께 받는 응답 형식은 공통 모델로 분리해 재사용했습니다.',
        ],
        result: [
          '무한 스크롤과 당겨서 새로고침을 적용해 목록을 이어서 조회하거나 첫 페이지부터 다시 불러올 수 있도록 했습니다.',
      ...
      },
      {
        ...
        result: [
          '입력 UI를 공통 구조로 정리해, 화면에서는 필요한 값만 전달해 사용할 수 있도록 했습니다.',
        ],
      },
      {
        title: '공통 테마와 스타일 적용',
        ...
        result: [
          '화면마다 공통 스타일 기준을 유지할 수 있도록 했습니다.',
