...
export const experiences: Experience[] = [
  {
    ...
    details: [
      {
        title: 'Riverpod + GoRouter 기반 화면 흐름과 API 레이어 분리',
        implementation: [
          '로그인 여부나 특정 상태에 따라 초기 진입 화면이 달라지는 흐름을 라우터에서 공통으로 처리했습니다.',
          '화면 간 이동은 `route name` 기준으로 호출하고, 전달 값은 `path parameter`와 `extra`로 나눠 처리했습니다.',
          '네트워크 요청은 `Dio`와 API 클래스, `provider`를 분리해 주입하는 구조로 구성했습니다.',
        ],
        result: [
          '공통 진입 규칙은 라우터에서, 데이터 요청 구조는 API 영역에서 처리하도록 했습니다.',
          '화면에서는 이동과 요청 호출만 담당하고, 진입 흐름과 요청 구조는 한곳에서 관리할 수 있었습니다.',
        ],
      },
      {
        ...
        implementation: [
          ...
          '중첩 응답 구조는 하위 모델로 나눠, 필요한 형태로 분리해 관리했습니다.',
        ],
        result: [
          '요청, 응답, 상태 모델의 역할을 나눠 관리하면서 모델 구조를 구분해 사용할 수 있도록 했습니다.',
      ...
      },
      {
        ...
        implementation: [
          ...
          '유효성 검사 로직은 공통 함수로 분리하고, 필드별 입력 문구와 함께 재사용할 수 있도록 했습니다.',
          '입력값 변화에 따라 버튼 노출과 에러 스타일을 제어했습니다.',
        ],
        result: [
          '검증 로직을 공통 함수로 분리해 필드마다 같은 규칙을 반복 작성하지 않도록 했습니다.',
          '입력 UI를 공통 위젯으로 분리해 화면에서는 필요한 값만 전달해 사용할 수 있도록 했습니다.',
        ],
      },
      {
        ...
        implementation: [
          ...
          '검색 조건과 페이지 번호를 함께 전달해 현재 조건 기준으로 목록을 조회했습니다.',
      ...
      },
      {
        ...
        implementation: [
          ...
          '색상, 간격 같은 스타일 값은 별도 상수로 분리했습니다.',
          '반복해서 사용하는 UI는 공통 위젯으로 분리하고, 화면에서는 필요한 값만 넘겨 사용했습니다.',
